...
import { triggerFileCreationAction } from "@/app/utils/walletActions";
...
const Sidebar = () => {
  ...
  const onFileCreate = async (fileName: string) => {
    try {
      // Trigger AO chain action before creating file
      await triggerFileCreationAction(fileName, activeTeam?.teamName);
